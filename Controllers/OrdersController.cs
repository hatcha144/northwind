...
namespace NorthwindDatabase.Controllers
{
    public class OrdersController : Controller
    {
        ...
        // GET: Orders
        public ActionResult Index(string option, string search, DateTime? searchtwo)
        {
            
            if(option == "ShipCity")
            {
                return View(db.Orders.Where(x => x.ShipCity.StartsWith(search) || search == null));
            }

            else if (option == "ShippedCountry")
            {
                return View(db.Orders.Where(x => x.ShipCountry.StartsWith(search) || search == null));
            }
            else
            {
                var sDate = db.Orders.Where(x => x.ShippedDate == searchtwo || searchtwo == null).ToList();
